...
async function onConfigChange(config) {
  ...
  h1.style.fontSize = `${baseSize * 3}px`;
...
// Inicialização do SDK (mantido para compatibilidade com o ambiente original)
...
navButtons.forEach(button => {
  button.addEventListener('click', () => {
    ...
    // Remove 'active' de todos os botões e reseta o background
    navButtons.forEach(btn => {
      btn.classList.remove('active');
      btn.style.background = 'transparent';
    });

    // Adiciona 'active' ao botão clicado e define o background de destaque
    button.classList.add('active');
    // Pega a configuração atual para aplicar a cor de destaque
